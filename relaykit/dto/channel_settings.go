...
const (
	// HTTPProtocolAuto 表示自动协商上游 HTTP 协议。
	HTTPProtocolAuto = "auto"
	// HTTPProtocolHTTP1 表示强制使用 HTTP/1.1。
	HTTPProtocolHTTP1 = "http1"
	// MaxHTTP2ConnectionShards 是单渠道允许配置的最大 HTTP/2 连接分片数。
	MaxHTTP2ConnectionShards = 8
)

// ValidateHTTPTransport 校验保存渠道时的 HTTP 传输配置。
