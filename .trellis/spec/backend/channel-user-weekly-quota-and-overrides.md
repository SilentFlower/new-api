...
- 旧周限检查在启用正数限额时失败关闭；新增统一策略入口还须能确认配置存在与否，配置读取故障同样失败关闭。不得把存储故障当作不限。
- `used >= limit` 返回稳定的 429 且禁止普通换渠重试；503 不降级。新策略允许的副作用前一次 HTTP 降级见 [周期预算与降级契约](./channel-period-budget-fallback.md)。
- 所有结算入口必须调用统一的 `RecordChannelUserQuotaUsage` 或 `RecordRelayChannelUserQuotaUsage`，保证一次正向消费同时进入日限与周限。

### 个人覆盖

- 覆盖只允许临时或永久提额，不允许降额。单项覆盖必须大于对应基础值（新策略日限采用当前时间规则基线），且不超过并发 `1000` 或额度 `common.MaxQuota`。
- 渠道默认值为不限（`<= 0`）时，不允许为该项设置个人覆盖；覆盖不能把不限改成有限。
- 无周期策略时生效值为 `max(base, override)`，未设置/到期/非法旧数据/读取失败回落默认。revision>0 时有效个人明确日周项优先于当前时间规则，读取故障返回错误，禁止静默放宽。
...
- 无周期策略时 Relay 读取覆盖失败可安全回落渠道默认值；有策略时严格失败关闭；管理员查询和写入必须暴露错误，不得伪装成功。
