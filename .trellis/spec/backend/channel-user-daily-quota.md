...
> 预算 v2 迁移说明：当前配置、Relay 统一判定、按行用量/提额与前端预算表以 [周期预算与降级契约](./channel-period-budget-fallback.md) 为准。本文保留旧版签名/API 供历史数据与回滚查阅；渠道日额度列已 json:"-"，旧日限上下文键、RelayInfo 限额快照和 user-daily-quota 接口已删除，不得重新接入。自然日 key、正向结算、退款不回退、金额换算及 Redis 持久化恢复约束继续适用；目标值调整改用预算行接口。
