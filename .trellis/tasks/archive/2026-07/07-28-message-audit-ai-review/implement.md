...
4. 实现固定资料集、合并上下文与受限 Tool
   - 根据每个 compressed 断点的 `parent_request_id` 和目标最新请求生成虚拟文件清单。
   - 默认 `merged` 模式把固定资料集合并为一次审核输入，并由服务端按已发送虚拟分片生成覆盖范围。
   - 实现只读 `list_files`、`read_file`、`search_files`，限制文件范围、游标、查询长度和返回量。
   - Tool 模式由服务端记录实际读取覆盖范围，不信任模型自报。
   - Tool 模式限制文件范围、游标、单次返回量、管理员配置的 Tool 调用次数和任务超时；累计 Tool Token 只用于诊断，真实上下文窗口由所选上游负责。

5. 实现默认提示词与结构化结果
   - 内置审核系统提示词、风险枚举、类别、注入防护和 JSON schema。
   - 实现 Tool 循环、最终 JSON 严格解析、范围校验和一次格式修复；Tool 模式读到材料后的结论阶段也启用 JSON 输出约束。
...
9. 修复失败原因和模型展示
   ...
   - 详情读取时按 Responses、Claude、Gemini 的 Tool 载荷派生语义角色和内容类型，兼容历史密文且不改变会话指纹。
...
13. 降低管理端统计与刷新压力
    ...
    - 审核渠道忽略原生 Tool 时由 relay 返回降级信号，service 构造严格 JSON 文本工具协议并继续执行文件范围、参数和调用次数校验。
...
15. 扩展图片请求审计
    - 将 `RelayFormatOpenAIImage` 加入消息审计协议允许列表。
    - 为 `dto.ImageRequest` 增加白名单规范化，只保存提示词和安全生成参数，明确排除图片、mask、Base64、媒体 URL 原文、额外字段和响应内容。
    - 图片协议跳过会话指纹生成，使每次图片请求独立成会话，同时保留同用户加密 blob 去重。
    - 增加协议、内容过滤和独立会话指纹回归测试。

16. 扩展审核上下文模式、Tool 与调用诊断
    - 保留字面量 `search_files`，新增受限内存 RE2 `search_files_regex`，补充分页游标、正则长度、文件范围和返回量校验。
    - 在审核固定配置中增加上下文模式和正整数 Tool 调用次数；上下文模式默认 `merged`，`tool_call_limit` 默认 24、不设固定最大值并兼容旧配置；删除独立累计 Tool Token 和本地估算输入 Token 停止条件，Tool 结果只返回剩余调用与累计 Token，模型真实上下文溢出由上游稳定归类。
   ...
   - 合并模式不注册 Tool，直接请求 JSON 结论；Tool 模式已有覆盖后的结论请求和格式修复请求设置 JSON 输出约束，降低 `invalid_output`。
...
- 管理端只配置固定渠道和模型，不提供自定义审核提示词；默认合并模式按已发送虚拟分片记录覆盖，Tool 模式按需读取虚拟分片并记录覆盖，达到上下文或 Tool 上限时明确失败。
...
- 图片生成和编辑请求保存提示词与安全参数，但不保存图片、mask、Base64、媒体 URL 原文、额外字段或生成结果；重复图片请求仍分别显示为独立会话。
