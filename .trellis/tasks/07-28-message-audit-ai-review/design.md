...
- 游标、查询长度、单次返回量、管理员配置的 Tool 调用次数和任务时长均受控。`message_audit_review.config.tool_call_limit` 接受任意正整数，默认 24，不设人为固定上限；触发任务时将归一化后的值固化进 payload。累计 Tool Token 和本地估算输入 Token 不再作为独立停止条件。
