...
const { CHANNEL_FORM_DEFAULT_VALUES, channelFormSchema } =
  await import('../../../../lib')
const { ChannelUserDailyQuotaLimitField } =
  await import('../channel-user-daily-quota-limit-field')
...
test('每日额度输入允许清空、重输 600 并按稳定步长提交', async () => {
  useSystemConfigStore.getState().setConfig({
    currency: { ...DEFAULT_CURRENCY_CONFIG },
  })
  ...
  assert.equal(submissions.at(-1)?.user_daily_quota_limit, 0)
  ...
  await flushAsyncWork()
  assert.equal(submissions.at(-1)?.user_daily_quota_limit, 600)
