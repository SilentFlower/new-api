...
/**
 * 渲染渠道单用户每周额度上限字段，并保留输入中的临时空值。
 *
 * @returns 与当前渠道表单绑定的每周额度输入字段。
 */
export function ChannelUserWeeklyQuotaLimitField() {
  ...
  return (
    <FormField
      ...
      name='user_weekly_quota_limit'
      render={({ field }) => (
        <FormItem>
          <FormLabel>
            {t('User weekly quota limit ({{unit}})', {
          ...
          <FormDescription>
            <span className='block'>
              {t(FIELD_DESCRIPTIONS.USER_WEEKLY_QUOTA_LIMIT)}
            </span>
            <span className='block'>
              {t(
