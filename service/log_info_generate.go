...
// @param other 最终写入消费或错误日志的附加字段。
func MergeContextLogOther(ctx *gin.Context, other map[string]interface{}) {
	...
	for key, value := range logOther {
		...
		sourceAdminInfo, ok := value.(map[string]interface{})
		if !ok || sourceAdminInfo == nil {
			continue
		}
		targetAdminInfo, ok := other["admin_info"].(map[string]interface{})
		if !ok || targetAdminInfo == nil {
			targetAdminInfo = map[string]interface{}{}
			other["admin_info"] = targetAdminInfo
		}
		for adminKey, adminValue := range sourceAdminInfo {
			targetAdminInfo[adminKey] = adminValue
		}
	}
}
...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	if relayInfo.IsModelMapped {
		...
		other["origin_model_name"] = relayInfo.OriginModelName
		other["upstream_model_name"] = relayInfo.UpstreamModelName
	}
	if billingModelName := relayInfo.BillingModelName(); billingModelName != "" {
	...
	if relayInfo.LimitFallback != nil {
		adminInfo["channel_limit_fallback"] = relayInfo.LimitFallback
	}
