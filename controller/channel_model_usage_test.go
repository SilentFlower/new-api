...
func TestChannelModelUsageManagementContract(t *testing.T) {
	...
	require.NoError(t, db.Create(&model.Channel{Id: 80, Name: "模型累计合同", Status: common.ChannelStatusEnabled, Models: "A,B", Group: "default"}).Error)
	...
	require.NoError(t, service.RecordChannelUserModelQuotaUsage(t.Context(), 80, 77, 60, "A"))
	...
	require.NoError(t, err)
	c, recorder := newChannelUserLimitTestContext(http.MethodPut, "/api/channel/80/period-policy", string(body), params)
	...
	usageContext, usageRecorder := newChannelUserLimitTestContext(http.MethodGet, "/api/channel/80/budgets/"+saved.Data.Config.Budgets[0].ID+"/usage?scope=user&p=1&page_size=20", "", append(params, gin.Param{Key: "budget_id", Value: saved.Data.Config.Budgets[0].ID}))
	...
	for _, extension := range []string{`"model_usage_tracking_enabled":null`, `"model_usage_tracking_enabled":"true"`, `"model_usage_tracking":null`, `"model_usage_tracking":{"first_enabled_at":1}`, `"unknown_tracking":true`} {
		...
		invalid, result := newChannelUserLimitTestContext(http.MethodPut, "/api/channel/80/period-policy", base, params)
