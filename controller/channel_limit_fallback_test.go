	...
	for i, path := range []string{"/v1/chat/completions", "/v1/messages", "/v1/responses"} {
		for _, stream := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/%t", path, stream), func(t *testing.T) {
				...
				_, err := service.SaveChannelPeriodPolicy(context.Background(), source.Id, appdto.ChannelPeriodPolicyInput{Config: budgetPoolDailyConfig(1, target.Id, "gpt-4o-mini")}, 1)
				require.NoError(t, err)
				...
				require.NoError(t, db.First(&user, 77).Error)
				assert.Equal(t, 1000000-logs[0].Quota, user.Quota)
				// 目标渠道没有预算行，渠道级个人日身份仍始终累计。
				targetUsed, _, _, statusErr := service.GetChannelUserDailyQuotaUsage(c, target.Id, 77)
				require.NoError(t, statusErr)
				assert.Equal(t, int64(logs[0].Quota), targetUsed)
				// 只改变目标账单快照不能污染来源累计。
