...
type TaskPrivateData struct {
	...
	ChannelUserDailyQuotaTracked bool                `json:"channel_user_daily_quota_tracked,omitempty"` // 提交时是否启用渠道用户每日额度追踪
}
...
type TaskBillingContext struct {
	...
	OriginModelName  string             `json:"origin_model_name,omitempty"`  // 用户请求的原始模型
	BillingModelName string             `json:"billing_model_name,omitempty"` // 本次实际用于计费的模型
...
func InitTask(platform constant.TaskPlatform, relayInfo *commonRelay.RelayInfo) *Task {
	...
	if relayInfo != nil && relayInfo.ChannelMeta != nil {
		privateData.ChannelUserDailyQuotaTracked = relayInfo.ChannelMeta.ChannelUserDailyQuotaLimit > 0
