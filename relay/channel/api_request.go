...
// applyUpstreamContentLength 使用 RelayInfo 中冻结的请求体大小补齐 Content-Length。
// 某些兼容路径仍会传入被包装为普通 io.Reader 的 BodyStorage，因此无法由 net/http 自动推断。
...
// keepUpstreamRedirectResponse 阻止 net/http 自动跟随上游重定向，并将原始 3xx 返回给 Relay。
