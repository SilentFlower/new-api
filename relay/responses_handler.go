...
func ResponsesHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if newAPIError := validateResponsesCompactEndpoint(info); newAPIError != nil {
		return newAPIError
	}

	responsesReq, newAPIError := responsesRequestForHandler(info)
	if newAPIError != nil {
		return newAPIError
	}
	...
	if info.UsesResponsesCompactEndpoint() {
		return postResponsesCompactEndpointQuota(c, info, usageDto)
	}

	if strings.HasPrefix(info.OriginModelName, "gpt-4o-audio") {
		service.PostAudioConsumeQuota(c, info, usageDto, "")
	} else {
		service.SetResponsesCompactAudit(c, info, responsesCompactAuditOutcome(info))
