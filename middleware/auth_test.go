...
func TestAdminAuthAllowsPATWithLegacyNewAPIUserHeader(t *testing.T) {
	setupDashboardAuthMiddlewareTest(t)
	token := "ai-fund-admin-pat"
	user := &model.User{
		Username: "ai-fund-admin", Password: "password-placeholder", Role: common.RoleAdminUser,
		Status: common.UserStatusEnabled, Group: "default", AccessToken: &token, AuthVersion: 1,
		AffCode: "middleware-aff-ai-fund-admin",
	}
	require.NoError(t, model.DB.Create(user).Error)
	router := gin.New()
	router.GET("/admin-data", AdminAuth(), func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"id": c.GetInt("id"), "use_access_token": c.GetBool("use_access_token")})
	})
	request := httptest.NewRequest(http.MethodGet, "/admin-data", nil)
	request.Header.Set("Authorization", "Bearer "+token)
	request.Header.Set("New-Api-User", "legacy-id-is-ignored")
	response := httptest.NewRecorder()
	...
	require.NoError(t, common.Unmarshal(response.Body.Bytes(), &body))
	assert.Equal(t, user.Id, body.ID)
	assert.True(t, body.UseAccessToken)
}

func TestTokenAuthReadOnlyAllowsAIKeyLogQuery(t *testing.T) {
	setupDashboardAuthMiddlewareTest(t)
	user := createMiddlewarePATUser(t, "api-key-log-user", "unrelated-dashboard-pat")
	token := &model.Token{
		UserId: user.Id,
		Key:    "readonlylogkey",
		Status: common.TokenStatusEnabled,
		Name:   "ai-fund-log-key",
	}
	require.NoError(t, model.DB.Create(token).Error)
	router := gin.New()
	router.GET("/api/log/token", TokenAuthReadOnly(), func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"user_id": c.GetInt("id"), "token_id": c.GetInt("token_id")})
	})
	request := httptest.NewRequest(http.MethodGet, "/api/log/token", nil)
	request.Header.Set("Authorization", "Bearer sk-"+token.Key+"-client-suffix")
	response := httptest.NewRecorder()
